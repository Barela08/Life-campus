...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    ...
    { to: '/teacher/leave', label: 'My Leave', icon: <CalendarCheck size={18} /> },
    ...
    { to: '/teacher/requests', label: 'Student Requests', icon: <ClipboardCheck size={18} /> },
  ]
      ...
      <aside className={cn(
        ...
        <div className="flex items-center gap-2 px-6 h-16 border-b border-gray-100 dark:border-gray-800">
          {systemLogo ? (
            ...
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white font-bold">
              {systemName.charAt(0)}
            </div>
          )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between px-4 h-16 glass border-b border-gray-100 dark:border-gray-800 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="lg:hidden font-bold truncate"><span className="text-indigo-600">{systemName}</span> Teacher</div>
          </div>
