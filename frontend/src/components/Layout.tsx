...
import {
  ...
  MonitorPlay, CalendarCheck, FileText, Bell, UserCircle, Settings,
  LogOut, Moon, Sun, Menu, X, ShieldCheck, AlertTriangle, Camera,
} from 'lucide-react'
...
export function Layout({ role, children }: { role: 'admin' | 'teacher' | 'student'; children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = role === 'admin' ? [
    ...
    { to: '/admin/attendance', label: 'Live Attendance', icon: <CalendarCheck size={18} /> },
    ...
    { to: '/admin/analytics', label: 'Analytics', icon: <Layers size={18} /> },
    ...
    { to: '/teacher/attendance', label: 'Start Attendance', icon: <Camera size={18} /> },
    ...
    { to: '/student/face', label: 'Face Registration', icon: <Camera size={18} /> },
  ...
  const topbar = (
    <div className="flex items-center justify-between px-4 h-16 glass border-b border-gray-100 dark:border-gray-800 sticky top-0 z-30">
      <div className="flex items-center gap-3">
        ...
        <div className="lg:hidden font-bold"><span className="text-primary-600">LifeOS</span> Campus</div>
      </div>
    ...
    <aside className={cn(
      ...
      <div className="flex items-center gap-2 px-6 h-16 border-b border-gray-100 dark:border-gray-800">
        ...
        <div>
          ...
          <p className="text-[10px] text-gray-400 uppercase tracking-widest">Smart Campus</p>
        </div>
      </div>
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            end={item.to.endsWith('/admin') || item.to.endsWith('/teacher') || item.to.endsWith('/student')}
