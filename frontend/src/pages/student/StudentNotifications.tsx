...
export default function StudentNotifications() {
  ...
  const load = async () => {
    setLoading(true)
    try { setItems((await api.get('/student/notifications')).data) } catch { toast.error('Failed') } finally { setLoading(false) }
  }
  useEffect(() => { load() }, [])

  const markRead = async (id: number) => {
    try { await api.post(`/student/notifications/${id}/read`, {}); load() } catch {}
  }
        ...
        <div className="space-y-3">
          {items.map(n => (
            <div key={n.id} className={`card p-4 flex items-start gap-3 animate-fade-in ${n.is_read ? 'opacity-60' : ''}`}>
              ...
              {!n.is_read && <button onClick={() => markRead(n.id)} className="p-1.5 rounded-lg hover:bg-emerald-50 text-emerald-600" title="Mark read"><CheckCircle2 size={16} /></button>}
            </div>
          ))}
        </div>
      )}
